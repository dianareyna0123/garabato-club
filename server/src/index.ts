...
import { DEFAULT_WORDS, buildTurnOrder, newRoom, normalizeAnswer, parseWords, roomView, scoreGuess, type Player, type Room } from './game.js';
...
function finishTurn(room:Room) {
  ...
  io.to(room.code).emit('turn:ended',{word:room.secretWord || room.options?.[0] || '', nextAt:Date.now()+3500});
  emitRoom(room); setTimeout(()=>startNextTurn(room),3500);
}
...
function startNextTurn(room:Room) {
  ...
  room.players.forEach(p=>p.guessed=false); room.secretWord=undefined; room.history=[]; room.phase='choosing';
...
io.on('connection', socket => {
  ...
  socket.on('game:start',()=>{ const c=current(socket); if(!c?.room || !c.room.players.get(c.playerId)?.isHost || c.room.phase!=='lobby') return; const connected=[...c.room.players.values()].filter(p=>p.connected); const needed=c.room.settings.wordOptions * (c.room.settings.allowRepeats?1:connected.length*c.room.settings.rounds); if(connected.length<2) return socket.emit('app:error','Se necesitan al menos 2 jugadores.'); if(wordPool(c.room).length<needed) return socket.emit('app:error',`Se necesitan al menos ${needed} palabras disponibles con esta configuración.`); c.room.players.forEach(p=>p.score=0); c.room.turnOrder=buildTurnOrder(connected.map(p=>p.id),c.room.settings.rounds); c.room.turnIndex=-1; c.room.usedWords.clear(); startNextTurn(c.room); });
...
app.use(express.static(clientDist)); app.get('*',(_req,res)=>res.sendFile(path.join(clientDist,'index.html')));
